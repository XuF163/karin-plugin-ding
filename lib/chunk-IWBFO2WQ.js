...
var DingTalkBot = class extends AdapterBase {
  ...
  async handleStreamEvent(topic, streamEvent) {
    ...
    if (!parsed.ok) {
      logger4.bot("warn", this.selfId, `[recv] invalid json: ${parsed.error.message}`);
      return;
    }
    ...
    if (scene === "group") {
      ...
      createGroupMessage({
        ...
        elements,
        srcReply: (els) => this.sendMsg(contact2, els)
      });
      return;
    }
    const userId = toStr(data?.senderStaffId || data?.senderId);
  ...
  async sendMsg(contact, elements, _retryCount = 0) {
    ...
    const sendImage = async (file) => {
      ...
      if (webhookCtx?.webhook) {
        if (this.enablePublicImageBed && /^https?:\/\//i.test(clean)) {
          const resp2 = await sendWebhookMarkdown({
            webhook: webhookCtx.webhook,
            secret: webhookCtx.secret,
            title: "\u56FE\u7247",
            text: `![\u56FE\u7247](${clean})
`
  ...
  async recallMsg(_contact, _messageId) {
  }
};
